...
public class LoggingAspect {
	...
	@Pointcut(value = "execution(* tn.enicarthage.projetspring.repositories.*.*(..))")
	public void forRepositoryLog() {}
	
	@Pointcut(value = "execution(* tn.enicarthage.projetspring.services.*.*(..))")
	public void forServiceLog () {}
	
	@Pointcut(value = "execution(* tn.enicarthage.projetspring.web.*.*(..))")
	...
	public void beforMethod(JoinPoint joinPoint) {
	  ...
	  @AfterReturning(pointcut = "forAllApp()", returning = "result")
	    public void afterReturningMethod(JoinPoint joinPoint, Object result) {
	        String methodName = joinPoint.getSignature().toShortString();
	        log.info("====> Execution of {} ended without exception", methodName);
	        if (result != null) {
	            log.info("====> Result: {}", result);
	        }
	    }

	    @AfterThrowing(pointcut = "forAllApp()", throwing = "ex")
